...
public class DefaultItemLockingServiceIntegrationTest extends ServicelayerBaseTest
{
	...
	@Test
	public void testSeflHealingWithLockedObjectWithSLD()
	{
		switcherForPersistence.switchToValue("false");
		testSeflHealingWithLockedObject();
		switcherForPersistence.switchBackToDefault();
	}

	@Test
	public void testSeflHealingWithLockedObjectWithJalo()
	{
		switcherForPersistence.switchToValue("true");
		testSeflHealingWithLockedObject();
		switcherForPersistence.switchBackToDefault();
	...
	public void testSeflHealingWithLockedObject()
	{
		//given
		user.setUid(UUID.randomUUID().toString());
		final CurrencyModel currencyModel = modelService.create(CurrencyModel._TYPECODE);
		currencyModel.setIsocode("xy");
		currencyModel.setSymbol("Xy");
		modelService.save(currencyModel);
		user.setSessionCurrency(currencyModel);
		modelService.save(user);
		...
		//then
		assertThat(userJalo.getProperty("sessionCurrency")).isNull();
	}
