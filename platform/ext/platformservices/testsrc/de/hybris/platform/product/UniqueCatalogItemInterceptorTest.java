...
public class UniqueCatalogItemInterceptorTest extends ServicelayerBaseTest
{
	...
	@Test
	public void testIfDefaultCatalagWasSet()
	{
		catalog.setDefaultCatalog(true);
		modelService.save(catalog);

		final ProductModel productModel = modelService.create(ProductModel.class);
		productModel.setCode("code1");
		modelService.save(productModel);

		assertThat(productModel.getCatalogVersion()).isNotNull();
	}

	@Test
	public void testSetDefaultCatalogBlacList()
	{
		catalog.setDefaultCatalog(true);
		modelService.save(catalog);

		final ClassificationClassModel modelToSave = modelService.create(ClassificationClassModel.class);
		modelToSave.setCode("newCode");
		...
		assertThat(expectedException).isInstanceOf(ModelSavingException.class);
	}
