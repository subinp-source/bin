...
public class CleanupDynamicProcessDefinitionsStrategyIntegrationTest extends ServicelayerBaseTest
{
	...
	public void shouldReturnResultWithoutGivenThresholds()
	{
		...
		assertThat(definitionsToRemove).hasSize(5)
		                               .is(containsOnlyHistorical())
		                               .is(containsDefinition(FIRST_DEFINITION, 1))
		                               .is(containsDefinition(FIRST_DEFINITION, 2))
		                               .is(containsDefinition(SECOND_DEFINITION, 1))
		                               .is(containsDefinition(SECOND_DEFINITION, 2))
		                               .is(containsDefinition(SECOND_DEFINITION, 3));
	}
	...
	private org.assertj.core.api.Condition<? super List<? extends DynamicProcessDefinitionModel>> containsOnlyHistorical()
	{
		return new org.assertj.core.api.Condition<>("Must contains only historical definitions")
		{
			@Override
			public boolean matches(final List<? extends DynamicProcessDefinitionModel> definitions)
			{
				for (final DynamicProcessDefinitionModel definition : definitions)
				{
	...
	private org.assertj.core.api.Condition<? super List<? extends DynamicProcessDefinitionModel>> containsDefinition(
			final String code, final long version)
	{
		return new org.assertj.core.api.Condition<>("Must contains definition with code: " + code + " and version: " + version)
		{
			@Override
			public boolean matches(final List<? extends DynamicProcessDefinitionModel> definitions)
			{
				for (final DynamicProcessDefinitionModel definition : definitions)
				{
					if (/* definition != null && */definition.getCode().equals(code)
							&& definition.getVersion().equals(version))
					{
						return true;
					}
				}
