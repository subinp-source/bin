...
public abstract class CatalogUnawarePDTRowTest<T extends PDTRowModel> extends ServicelayerBaseTest
{
	...
	public void shouldSynchronizeRowForProduct()
	{
		...
		assertThat(syncRows).hasSize(1).doesNotContain(srcRow);
		final T tgtRow = syncRows.get(0);
		assertThat(tgtRow).has(isSynchronizedVersionOf(srcRow));
	}

	private Condition<? super T> isSynchronizedVersionOf(final T srcRow)
	{
		return new Condition<>()
		{
			...
			{
				assertThatTgtRowMatchSrcRow(value, srcRow);
				return true;
			}
		};
	}
