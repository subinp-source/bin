			...
			final RedirectAttributes redirectModel) throws CMSItemNotFoundException
	{
		...
		if (cookie != null)
		{
			try
			{
				...
				final List<AnonymousConsentData> consentCookieDataList = Arrays.asList(mapper.readValue(
						URLDecoder.decode(cookie.getValue(), StandardCharsets.UTF_8.displayName()), AnonymousConsentData[].class));
				consentCookieDataList.stream().filter(consentData -> CONSENT_GIVEN.equals(consentData.getConsentState()))
