...
public class RuleEngineRuleValidateInterceptorUnitTest
{
	...
	public void testFailOnValidateNewActiveNotSet() throws InterceptorException
	{
		//given
		given(valueOf(context.isNew(model))).willReturn(Boolean.TRUE);
		given(l10NService.getLocalizedString("exception.ruleenginerulevalidateinterceptor", new Object[]{model,"Active flag must be set"})).willReturn("'Active flag must be set'");
	...
	public void testFailOnValidateNewMustBeActive() throws InterceptorException
	{
		//given
		given(valueOf(context.isNew(model))).willReturn(Boolean.TRUE);
		given(l10NService.getLocalizedString("exception.ruleenginerulevalidateinterceptor", new Object[]{model,"Non active rule version cannot increase overall knowledgebase snapshot version"})).
				willReturn("'Non active rule version cannot increase overall knowledgebase snapshot version'");
	...
	public void testFailOnValidateModifiedNonActiveRuleVersionsNotEqual() throws InterceptorException
	{
		...
		given(valueOf(context.isModified(model))).willReturn(Boolean.TRUE);
		model.setActive(Boolean.FALSE);
		given(l10NService.getLocalizedString("exception.ruleenginerulevalidateinterceptor", new Object[]{model,"Historical version of the rule cannot be modified"})).
				willReturn("'Historical version of the rule cannot be modified'");
		//when
		final Throwable throwable = ThrowableAssert.catchThrowable(() -> validateInterceptor.onValidate(model, context));
		//then
		assertThat(throwable).isInstanceOf(InterceptorException.class)
	...
	public void testFailOnValidateModifiedActiveRuleNoContent() throws InterceptorException
	{
		...
		given(valueOf(context.isModified(model))).willReturn(Boolean.TRUE);
		model.setActive(Boolean.TRUE);
		given(l10NService.getLocalizedString("exception.ruleenginerulevalidateinterceptor", new Object[]{model,"Checksum doesn't match the rule content. Expected null but was RULE_CHECKSUM"})).
				willReturn("'Checksum doesn't match the rule content. Expected null but was RULE_CHECKSUM'");
		//when
		final Throwable throwable = ThrowableAssert.catchThrowable(() -> validateInterceptor.onValidate(model, context));
		//then
		assertThat(throwable).isInstanceOf(InterceptorException.class)
	...
	public void testFailOnValidateModifiedActiveRuleWithContentWrongChecksum() throws InterceptorException
	{
		...
		given(valueOf(context.isModified(model))).willReturn(Boolean.TRUE);
		model.setActive(Boolean.TRUE);
		given(l10NService.getLocalizedString("exception.ruleenginerulevalidateinterceptor", new Object[]{model,"Checksum doesn't match the rule content. Expected RULE_CHECKSUM but was WRONG_CHECKSUM"})).
				willReturn("'Checksum doesn't match the rule content. Expected RULE_CHECKSUM but was WRONG_CHECKSUM'");
		//when
		final Throwable throwable = ThrowableAssert.catchThrowable(() -> validateInterceptor.onValidate(model, context));
		//then
		assertThat(throwable).isInstanceOf(InterceptorException.class)
	...
	public void testFailOnValidateRemovedActiveRule() throws InterceptorException
	{
		...
		given(l10NService.getLocalizedString("exception.ruleenginerulevalidateinterceptor", new Object[]{model,"Rule must be active."})).
				willReturn("'Rule must be active.'");
		model.setActive(Boolean.TRUE);
		//when
		final Throwable throwable = ThrowableAssert.catchThrowable(() -> validateInterceptor.onValidate(model, context));
		//then
	...
	public void testValidateNoKieBase() throws InterceptorException
	{
		...
		model.setActive(Boolean.TRUE);
		given(l10NService.getLocalizedString("exception.ruleenginerulevalidateinterceptor", new Object[]{model,"rule must have correct associated KieBase"})).
				willReturn("rule must have correct associated KieBase");
		//when
		final Throwable throwable = ThrowableAssert.catchThrowable(() -> validateInterceptor.onValidate(model, context));
		//then
	...
	public void testValidateNoKieModule() throws InterceptorException
	{
		...
		given(l10NService.getLocalizedString("exception.ruleenginerulevalidateinterceptor", new Object[]{model,"rule must have correct associated KieModule"})).
				willReturn("rule must have correct associated KieModule");
		model.setActive(Boolean.TRUE);
	...
	public void testFailOnValidateNewManuallyCreatedRuleWithOldVersionVersion() throws InterceptorException
	{
		...
		given(l10NService.getLocalizedString("exception.ruleenginerulevalidateinterceptor", new Object[]{model,"Rule must be created using latest rule module version."})).
				willReturn("Rule must be created using latest rule module version.");
		//when
		final Throwable throwable = ThrowableAssert.catchThrowable(() -> validateInterceptor.onValidate(model, context));
		//then
		assertThat(throwable).isInstanceOf(InterceptorException.class)
	...
	public void testFailOnValidateNewManuallyCreatedRuleWithContentWrongChecksum() throws InterceptorException
	{
		...
		given(l10NService.getLocalizedString("exception.ruleenginerulevalidateinterceptor", new Object[]{model,"Checksum doesn't match the content."})).
				willReturn("Checksum doesn't match the content.");
		//when
		final Throwable throwable = ThrowableAssert.catchThrowable(() -> validateInterceptor.onValidate(model, context));
		//then
		assertThat(throwable).isInstanceOf(InterceptorException.class)
