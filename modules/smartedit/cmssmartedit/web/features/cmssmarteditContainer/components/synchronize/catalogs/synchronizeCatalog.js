 ...
 */
angular
    .module('synchronizeCatalogModule', ['seConstantsModule'])

    .controller('synchronizeCatalogController', function(
        $scope,
        ...
        this.$onInit = function() {
            ...
            synchronizationService.startAutoGetSyncData(
                this.catalogDto,
                this._updateSyncStatusData.bind(this)
            );
        ...
        this.syncCatalog = function() {
            ...
            confirmationModalService
                .confirm({
                    template:
                        '<div id="confirmationModalDescription">{{ "se.sync.confirm.msg" | translate: { catalogName: modalController.catalogName } }}</div>',
                ...
                .then(
                    function() {
                        synchronizationService.updateCatalogSync(this.catalogDto).then(
                            function(response) {
        ...
        this._invokeGetSyncData = function() {
            synchronizationService.getCatalogSyncStatus(this.catalogDto).then(
                function(response) {
                    this._updateSyncStatusData(response);
                }.bind(this)
            );
        ...
        this.isSyncJobInProgress = function() {
            return (
                this.syncJobStatus.status === 'RUNNING' || this.syncJobStatus.status === 'UNKNOWN'
            );
        };
