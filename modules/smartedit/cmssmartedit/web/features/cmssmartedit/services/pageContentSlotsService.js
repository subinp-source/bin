 ...
 */
angular
    .module('pageContentSlotsServiceModule', [
        'resourceModule',
        'smarteditServicesModule',
        'cmsResourceLocationsModule',
        'yLoDashModule'
    ])
    .service('pageContentSlotsService', function(
        $q,
        lodash,
        restServiceFactory,
        ...
        this._reloadPageContentSlots = function() {
            return pageInfoService.getPageUID().then(function(pageId) {
                pageContentSlotsPromise = pagesContentSlotsResource
                    .get({
                        pageId: pageId
                    })
                    .then(function(pagesContentSlotsResponse) {
                        return $q.when(
                            lodash.uniq(pagesContentSlotsResponse.pageContentSlotList || [])
                        );
                    });
        ...
        this.getPageContentSlots = function() {
            return pageContentSlotsPromise
                ? $q.when(pageContentSlotsPromise)
        ...
        this.getSlotStatus = function(slotId) {
            return (pageContentSlotsPromise
                ? $q.when(pageContentSlotsPromise)
                : this._reloadPageContentSlots()
            ).then(function(pageContentSlots) {
