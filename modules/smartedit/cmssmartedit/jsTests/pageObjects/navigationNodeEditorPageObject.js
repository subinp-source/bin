...
navigationNodeEditor.prototype = {
    ...
    clickMoreMenuOptionByIndex: function(entryPosition, text) {
        return this.yDropDownComponent.actions.selectLabel(
            '.nav-node-editor-entry-item:nth-child(' + (entryPosition + 2) + ')',
            text
        );
    },
    ...
    selectTypeOption: function(dropdown, type) {
        ...
        if (dropdown === 'itemId') {
            option = element
                .all(
                    by.xpath(
                        "//*[@id = '" +
                            dropdown +
                            "-list']//se-infinite-scrolling//*[contains(text(),'" +
                            type +
                            "')]"
                    )
                )
            ...
            option = element
                .all(
                    by.xpath(
                        "//*[@id = '" + dropdown + "-list']//*[contains(text(),'" + type + "')]"
                    )
                )
                .first();
        }
    ...
    selectOption: function(dropdown, index) {
        ...
        if (dropdown === 'itemId') {
            option = element(
                by.xpath(
                    "//ul[@id = '" + dropdown + "-list']//se-infinite-scrolling//li[" + index + ']'
                )
            );
        } else {
            option = element(
                by.xpath("//ul[@id = '" + dropdown + "-list']/li/ul/li[" + index + ']')
            );
        }
        ...
        }, "Option at index '" + index + "'not clickable");
    },
    selectOptionByLabel: function(dropdown, label, isNotPaged) {
        ...
        if (!isNotPaged) {
            ...
            option = element
                .all(
                    by.xpath(
                        "//*[@id = '" + dropdown + "-list']//*[contains(text(),'" + label + "')]"
                    )
                )
                .first();
        }
