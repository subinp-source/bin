...
xdescribe('slotVisibilityButton', function() {
    ...
    describe('when hidden component list is not empty', function() {
        describe('template', function() {
            it('should display the hidden components button', function() {
                ...
                // Assert
                expect(
                    element.find('button.se-slot-ctx-menu__dropdown-toggle--slot-visibility').length
            ...
            it('should display dropdown menu if the button is clicked', function() {
                ...
                expect(
                    element.find(
                        '.se-slot-ctx-menu__dropdown-toggle-wrapper--slot-visibility .se-slot-visility__component-list'
                    ).length
                ).toBe(1);
            });
    ...
    function buildComponentWithNoHiddenComponents() {
        ...
        return AngularUnitTestHelper.prepareModule('slotVisibilityButtonModule')
            ...
            .and.returnResolvedPromise([])
            .mock('catalogVersionPermissionService', 'hasWritePermission')
            .and.returnResolvedPromise(true)
            .mock('gatewayProxy', 'initForService')
            .and.returnValue()
            .mock('slotVisibilityService', 'getHiddenComponents')
            .and.returnResolvedPromise([])
    ...
    function buildComponentWithTwoHiddenComponents() {
        ...
        return AngularUnitTestHelper.prepareModule('slotVisibilityButtonModule')
            ...
            .mock('editorModalService', 'openand.rerenderSlot')
            .and.returnResolvedPromise()
            .mock('crossFrameEventService', 'subscribe')
            .and.returnValue(function() {})
            .mockConstant('EVENT_OUTER_FRAME_CLICKED', 'EVENT_OUTER_FRAME_CLICKED')
