...
describe('homepageService', () => {
    ...
    const eventService: jasmine.SpyObj<SystemEventService> = jasmine.createSpyObj('eventService', [
        'publish'
    ]);
    ...
    beforeEach(() => {
        ...
        catalogService.getContentCatalogVersion.and.callFake((urlContext: IUriContext) => {
            ...
            if (!matchingCatalog) {
                throw Error(
                    `HomepageServiceTest - no catalog found with ID ${
                        urlContext.CURRENT_CONTEXT_CATALOG
                    }`
                );
            }
    ...
    it('isCurrentHomepage - true', (done) => {
        ...
        homepageService.isCurrentHomepage(page, uriContexts.ukStaged).then((result: boolean) => {
            expect(homepageService.getHomepageType).toHaveBeenCalledWith(
                page,
                uriContexts.ukStaged
            );
    ...
    it('hasFallbackHomePage - false', (done) => {
        homepageService
            .hasFallbackHomePage(uriContexts.electronicsStaged)
            .then((result: boolean) => {
                expect(result).toBe(false);
                done();
            });
    ...
    describe('getHomepageDetailsForContext', () => {
        ...
        it('Handles failure - getCatalogVersionByUuid', () => {
            ...
            expect(
                homepageService.getHomepageDetailsForContext(uriContexts.ukStaged)
            ).toBeRejectedWithData(errMsg);
        });
        ...
        it('NO homepage', () => {
            expect(
                homepageService.getHomepageDetailsForContext(uriContexts.noHomepageOnline)
        ...
        it('Is a PARENT homepage', () => {
            ...
            expect(
                homepageService.getHomepageDetailsForContext(uriContexts.frStaged)
            ).toBeResolvedWithData({
                status: 'PARENT',
                parentCatalogName: {
