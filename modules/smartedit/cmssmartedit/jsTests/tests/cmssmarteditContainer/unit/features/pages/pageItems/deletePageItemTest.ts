...
describe('DeletePageItemController', () => {
    ...
    beforeEach(() => {
        MockedManagePageService = jasmine.createSpyObj<ManagePageService>(
            'MockedManagePageService',
            ['isPageTrashable', 'getDisabledTrashTooltipMessage', 'softDeletePage']
        );
        ...
        controller = new DeletePageItemComponent(
            MockedManagePageService,
            MockedSystemEventService,
    ...
    it('$onInit sets "isPageTrashable" with result returned by deletePageService for a given pageUid', function() {
        ...
        MockedManagePageService.isPageTrashable.and.returnValue(
            $q.resolve(MOCKED_IS_PAGE_TRASH_RESPONSE)
        );
        ...
        // ASSERT
        expect(MockedManagePageService.isPageTrashable).toHaveBeenCalledWith(
            MOCKED_PAGE_INFO,
            MOCKED_URI_CONTEXT
        );
    ...
    it('onClickOnDeletePage sends an event when the soft deletion of a page is resolved', function() {
        // GIVEN
        MockedCatalogService.retrieveUriContext.and.returnValue($q.resolve(MOCKED_URI_CONTEXT));
        MockedManagePageService.softDeletePage.and.returnValue(
            $q.resolve(MOCKED_IS_PAGE_TRASH_RESPONSE)
        );
        ...
        // ASSERT
        expect(MockedManagePageService.softDeletePage).toHaveBeenCalledWith(
            MOCKED_PAGE_INFO,
            MOCKED_URI_CONTEXT
        );
        expect(MockedSystemEventService.publishAsync).toHaveBeenCalledWith(
            MOCKED_EVENT_CONTENT_CATALOG_UPDATE,
            MOCKED_IS_PAGE_TRASH_RESPONSE
        );
    });
});
