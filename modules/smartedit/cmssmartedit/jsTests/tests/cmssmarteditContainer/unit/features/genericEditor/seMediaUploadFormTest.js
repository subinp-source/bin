...
describe('seMediaUploadForm', function() {
    ...
    beforeEach(
        angular.mock.module('seBackendValidationHandlerModule', function($provide) {
            seBackendValidationHandler = jasmine.createSpyObj('seBackendValidationHandler', [
                'handleResponse'
            ]);
    ...
    beforeEach(
        angular.mock.module('seObjectValidatorFactoryModule', function($provide) {
    ...
    describe('initialization', function() {
        ...
        it('should show translated labels for each of the fields', function() {
            expect(
                element
                    .find('label[name="label-description"]')
                    .parent()
                    .text()
            ).toContain('Description');
            expect(
                element
                    .find('label[name="label-code"]')
                    .parent()
                    .text()
            ).toContain('Code');
            expect(
                element
                    .find('label[name="label-alt-text"]')
                    .parent()
                    .text()
            ).toContain('Alt Text');
        });
    ...
    describe('on upload with invalid parameters', function() {
        ...
        it('should show validation errors for each field', function() {
            expect(element.find('span.upload-field-error-description').text()).toBe(
                'some description message'
            );
            expect(element.find('span.upload-field-error-code').text()).toBe('some code message');
            expect(element.find('span.upload-field-error-alt-text').text()).toBe(
                'some alt text message'
            );
        });
    });
    ...
    describe('on upload', function() {
        ...
        describe('on failure', function() {
            beforeEach(function() {
                seBackendValidationHandler.handleResponse.and.callFake(function(
                    response,
                    errorsContext
                ) {
                    errorsContext.push({
            ...
            it('should display validation errors from the backend', function() {
                expect(element.find('.upload-field-error-description').text()).toBe(
                    'some backend description message'
                );
                expect(element.find('.upload-field-error-code').text()).toBe(
                    'some backend code message'
                );
                expect(element.find('.upload-field-error-alt-text').text()).toBe(
                    'some backend alt text message'
                );
            });
        });
    });
