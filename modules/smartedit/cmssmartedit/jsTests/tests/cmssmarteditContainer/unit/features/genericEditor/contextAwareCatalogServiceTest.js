...
describe('contextAwareCatalog', function() {
    ...
    beforeEach(function() {
        searchResultUri = SEARCH_RESOURCE_URI.replace(/:catalogId/gi, catalogId).replace(
            /:catalogVersion/gi,
            catalogVersion
        );
        ...
        var harness = AngularUnitTestHelper.prepareModule('contextAwareCatalogModule')
            .mock('catalogService', 'getActiveProductCatalogVersionByCatalogId')
            .and.returnResolvedPromise(catalogVersion)
            .mock('catalogService', 'getActiveContentCatalogVersionByCatalogId')
            .and.returnResolvedPromise(catalogVersion)
            ...
            .mockConstant('PRODUCT_CATEGORY_RESOURCE_BASE_URI', ITEM_RESOURCE_URI)
            .mockConstant('PRODUCT_LIST_RESOURCE_API', SEARCH_RESOURCE_URI)
            .mockConstant('PRODUCT_RESOURCE_API', ITEM_RESOURCE_URI + '/:productUID')
    ...
    it('should be able to return proper uri for product category list', function() {
        expect(
            contextAwareCatalogService.getProductCategorySearchUri(catalogId)
        ).toBeResolvedWithData(searchResultUri);
    });

    it('should be able to return proper uri for product category item', function() {
        expect(contextAwareCatalogService.getProductCategoryItemUri()).toBeResolvedWithData(
            itemResultUri
        );
    });
    ...
    it('should be able to return proper uri for content page list', function() {
        ...
        expect(contextAwareCatalogService.getContentPageSearchUri()).toBeResolvedWithData(
            searchResultUri
        );
    });

    it('should be able to return proper uri for content page item', function() {
        expect(contextAwareCatalogService.getContentPageItemUri()).toBeResolvedWithData(
            searchResultUri
        );
    });

    it('should be able to return proper uri for product list', function() {
        expect(contextAwareCatalogService.getProductSearchUri(catalogId)).toBeResolvedWithData(
            searchResultUri
        );
    });
