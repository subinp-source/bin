...
module.exports = (function() {
    ...
    componentObject.elements = {
        ...
        getDropdownByQualifier: function(fieldQualifier) {
            return genericEditorCommons.elements
                ...
                .element(by.css('cms-item-dropdown'));
        },
        ...
        getDropdownOptionById: function(fieldQualifier, itemId) {
            return this.getDropdownByQualifier(fieldQualifier).element(
                by.css(
                    '.ui-select-choices-group nested-component [data-component-id="' + itemId + '"]'
                )
            );
        },
        getSingleSelectSelectedItemById: function(fieldQualifier, itemId) {
            return this.getDropdownByQualifier(fieldQualifier).element(
                by.css(
                    '.ui-select-match .y-select-item-printer nested-component [data-component-id="' +
                        itemId +
                        '"]'
                )
            );
        ...
        getSingleSelectSelectedItemImageById: function(fieldQualifier, itemId) {
            return this.getSingleSelectSelectedItemById(fieldQualifier, itemId).element(
                by.css('.cms-nested-component__item-icon')
            );
        },
        getSearchBox: function(fieldQualifier, selectType) {
            if (selectType === componentObject.constants.DROPDOWN_TYPE.SINGLE_SELECT) {
                ...
                return this.getDropdownByQualifier(fieldQualifier).element(
                    by.css('.select2-search-field input')
                );
            }
        },
        getCreateNewComponentButton: function(fieldQualifier) {
            return this.getDropdownByQualifier(fieldQualifier).element(
                by.css('.se-actionable-search-item__create-btn')
            );
        },
        getComponentSubType: function(componentType) {
            return element(
                by.cssContainingText(
                    'sub-type-selector .cms-sub-type__selector-link',
                    componentType
                )
            );
        },
        ...
        getRemoveButton: function() {
            return element(by.css('.se-generic-editor-dropdown__remove-button'));
        },
        getSelectedItemsList: function(fieldQualifier) {
            return this.getDropdownByQualifier(fieldQualifier).all(
                by.css('.select2-choices li.ui-select-match-item nested-component')
            );
        },
        getMultiSelectSelectedItem: function(fieldQualifier, componentId) {
            return this.getDropdownByQualifier(fieldQualifier).element(
                by.css(
                    '.select2-choices li.ui-select-match-item nested-component [data-component-id="' +
                        componentId +
                        '"]'
                )
            );
        ...
        getSelectedItemRemoveButton: function(fieldQualifier, componentId) {
            ...
            var xPathExpression =
                '//li[contains(@class, "ui-select-match-item")][//nested-component/*[@data-component-id="' +
                componentId +
                '"]]/a';
    ...
    componentObject.actions = {
        ...
        openDropdown: function(fieldQualifier, selectType) {
            if (selectType === componentObject.constants.DROPDOWN_TYPE.SINGLE_SELECT) {
                return browser.click(
                    componentObject.elements.getSingleSelectOpenDropdownButton(fieldQualifier)
                );
        ...
        searchComponentInDropdown: function(fieldQualifier, selectType, componentNameToSearch) {
            return this.openDropdown(fieldQualifier, selectType).then(function() {
                return componentObject.elements
                    .getSearchBox(fieldQualifier, selectType)
                    .clear()
        ...
        clickDropdownOption: function(fieldQualifier, itemId) {
            return browser.click(
                componentObject.elements.getDropdownOptionById(fieldQualifier, itemId)
            );
        },
        selectItemInDropdown: function(fieldQualifier, selectType, itemId) {
            return this.openDropdown(fieldQualifier, selectType).then(
                function() {
        ...
        clickSelectedItem: function(fieldQualifier, selectType, itemId) {
            if (selectType === componentObject.constants.DROPDOWN_TYPE.SINGLE_SELECT) {
                ...
                return browser.click(
                    componentObject.elements.getMultiSelectSelectedItem(fieldQualifier, itemId)
                );
            }
        },
        ...
        removeSelectedItem: function() {
            return browser.click(componentObject.elements.getRemoveButton());
        }
    };
    ...
    componentObject.assertions = {
        ...
        isEmpty: function(fieldQualifier, selectType) {
            ...
            if (selectType === componentObject.constants.DROPDOWN_TYPE.SINGLE_SELECT) {
                expect(
                    browser.isPresent(
                        componentObject.elements.getDropdownEmptyLabel(fieldQualifier)
                    )
                ...
                expect(componentObject.elements.getSelectedItemsList(fieldQualifier).count()).toBe(
                    0,
                    'Expected dropdown ' + fieldQualifier + ' to be empty'
                );
            }
        },
        isNotEmpty: function(fieldQualifier, selectType) {
            ...
            if (selectType === componentObject.constants.DROPDOWN_TYPE.SINGLE_SELECT) {
                expect(
                    browser.isAbsent(componentObject.elements.getDropdownEmptyLabel(fieldQualifier))
                ).toBe(true, 'Expected dropdown ' + fieldQualifier + ' not to be empty');
            } else {
                expect(
                    componentObject.elements.getSelectedItemsList(fieldQualifier).count()
                ).not.toBe(0, 'Expected dropdown ' + fieldQualifier + ' not to be empty');
            }
        },
        hasRemoveButton: function() {
            return browser.waitForPresence(componentObject.elements.getRemoveButton());
        },
        itemIsSelected: function(fieldQualifier, selectType, itemId) {
            if (selectType === componentObject.constants.DROPDOWN_TYPE.SINGLE_SELECT) {
                expect(
                    componentObject.elements
                        .getSingleSelectSelectedItemById(fieldQualifier, itemId)
                        .isPresent()
