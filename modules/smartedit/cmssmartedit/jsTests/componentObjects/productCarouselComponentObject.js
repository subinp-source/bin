...
module.exports = (function() {
    ...
    componentObject.actions = {
        ...
        setProducts: function(productsList) {
            return catalogAwareSelector.actions
                .selectItems(componentObject.constants.PRODUCT_SELECTOR_ID, productsList)
                .then(function() {
        ...
        setCategories: function(categoriesList) {
            return catalogAwareSelector.actions
                .selectItems(componentObject.constants.CATEGORY_SELECTOR_ID, categoriesList)
                .then(function() {
                    return catalogAwareSelector.actions.clickAddItemsButton();
                });
        },
        setProductCarouselData: function(productCarouselData) {
            return componentObject.actions
                .setTitle(productCarouselData.title)
                .then(function() {
        ...
        moveProductUp: function(itemIndex) {
            return catalogAwareSelector.actions.moveItemUp(
                componentObject.constants.PRODUCT_SELECTOR_ID,
                itemIndex
            );
        },
        moveProductDown: function(itemIndex) {
            return catalogAwareSelector.actions.moveItemDown(
                componentObject.constants.PRODUCT_SELECTOR_ID,
                itemIndex
            );
        },
        deleteProduct: function(itemIndex) {
            return catalogAwareSelector.actions.deleteItem(
                componentObject.constants.PRODUCT_SELECTOR_ID,
                itemIndex
            );
        },
        moveProductToPosition: function(originalIndex, newIndex) {
            return catalogAwareSelector.actions.dragAndDropItemToPosition(
                componentObject.constants.PRODUCT_SELECTOR_ID,
                originalIndex,
                newIndex
            );
    ...
    componentObject.assertions = {
        ...
        hasTitle: function(expectedTitle) {
            genericEditor.assertions.localizedFieldHasExpectedValues(
                componentObject.constants.TITLE_FIELD_ID,
                expectedTitle
            );
        },
        hasProducts: function(expectedProductsList) {
            catalogAwareSelector.assertions.itemsAreSelected(
                componentObject.constants.PRODUCT_SELECTOR_ID,
        ...
        productsAreInRightOrder: function(expectedProductsList) {
            catalogAwareSelector.assertions.itemsAreInRightOrder(
                componentObject.constants.PRODUCT_SELECTOR_ID,
                expectedProductsList
            );
        },
