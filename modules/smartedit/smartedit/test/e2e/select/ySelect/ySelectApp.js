...
angular
    ...
    .run(function($templateCache) {
        $templateCache.put(
            'test.html',
            '<div><b>Label:</b> {{item.label}} <b>Price:</b> {{item.price}}</div>'
        );
    })
        ...
        this.changeSource = function(componentName) {
            ...
            if (componentName === 'exampleMulti1') {
                sources[componentName] =
                    sources[componentName] === productsV1 ? productsV2 : productsV1;
            } else {
                sources[componentName] =
                    sources[componentName] === languagesV1 ? languagesV2 : languagesV1;
            }
        };
