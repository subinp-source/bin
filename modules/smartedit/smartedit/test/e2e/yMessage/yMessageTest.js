...
describe('yMessage', function() {
    ...
    it(
        'GIVEN a page with a yMessage ' +
            'THEN the yMessage is displayed with a title and description',
        function() {
            //THEN
            yMessage.assertions.assertTitleContainsText(
                yMessage.constants.YMESSAGE_WITH_TYPE_ID,
                'TEST TITLE'
            );
            yMessage.assertions.assertDescriptionContainsText(
                yMessage.constants.YMESSAGE_WITH_TYPE_ID,
                'TEST DESCRIPTION'
            );
        }
    ...
    it(
        ...
        function() {
            //THEN
            yMessage.assertions.assertInfoTypeWasApplied(
                yMessage.constants.YMESSAGE_WITHOUT_TYPE_ID
            );
        }
    );
    ...
    it(
        ...
        function() {
            //THEN
            yMessage.assertions.assertComplexDescriptionWasTranscluded(
                yMessage.constants.YMESSAGE_WITH_COMPLEX_DESCRIPTION
            );
        }
    );
