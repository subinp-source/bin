...
export type ISeDropdownSelectedOption = GenericEditorOption;
export interface ISeDropdownSelectedOptionEventData<
    T extends ISeDropdownSelectedOption = ISeDropdownSelectedOption
> {
    qualifier: string;
    optionObject: T;
}
...
export const SEDropdownServiceFactory = (
    ...
    return class implements ISEDropdownService {
        ...
        triggerAction() {
            ...
            const handle: ISeDropdownSelectedOptionEventData = {
        ...
        fetchPage(search: string, pageSize: number, currentPage: number) {
            return this.populator
                ...
                .catch((err) => {
                    $log.error(
                        `SEDropdownService.fetchPage() - Failed to fetch items and paging information. ${err}`
                    );
                });
        }
        ...
        init(): void {
            ...
            if (this.field.options && this.field.uri) {
                ...
                $injector.has(this.field.cmsStructureType + DROPDOWN_IMPLEMENTATION_SUFFIX)
            ) {
