...
public class ConsignmentsControllerIntegrationTest extends BaseWarehousingWebservicesIntegrationTest
{
	...
	public void getAllConsignments()
	{
		...
		assertEquals(1,result.getConsignments().stream().filter(c -> c.getCode().contains(CONSIGNMENT_0)).
				count());
		assertEquals(1,result.getConsignments().stream().filter(c -> c.getCode().contains(CONSIGNMENT_1)).
				count());
	}
	...
	public void getConsignmentStatus()
	{
		...
		//then
		assertTrue(result.getStatuses().size() >= 14);
