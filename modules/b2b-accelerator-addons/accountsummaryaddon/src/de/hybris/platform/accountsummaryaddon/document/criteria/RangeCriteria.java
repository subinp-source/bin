...
public class RangeCriteria extends DefaultCriteria
{
	...
	public void populateCriteriaQueryAndParamsMap(final List<String> whereQueryList, final Map<String, Object> queryParamsMap)
	{
		...
		if (this.getStartRange().isPresent())
		{
			...
			{
				LOG.debug(String.format(QUERY_CRITERIA, formattedQuery, this.getStartRange().get()));	// NOSONAR
			}
			whereQueryList.add(formattedQuery);
			queryParamsMap.put("startRange", this.getStartRange().get());	// NOSONAR
		}
		...
		if (this.getEndRange().isPresent())
		{
			...
			if (LOG.isDebugEnabled())
			{
				LOG.debug(String.format(QUERY_CRITERIA, formattedQuery, this.getEndRange().get()));		// NOSONAR
			}
			whereQueryList.add(formattedQuery);
			queryParamsMap.put("endRange", this.getEndRange().get());	// NOSONAR
		}
