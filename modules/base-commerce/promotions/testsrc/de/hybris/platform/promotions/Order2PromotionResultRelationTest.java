...
public class Order2PromotionResultRelationTest extends AbstractPromotionServiceTest
{
	...
	{
		final String query = "GET {" + PromotionResultModel._TYPECODE + "}";
		final FlexibleSearchQuery searchQuery = new FlexibleSearchQuery(query);
		final List<Class> resultClassList = new ArrayList<Class>();
		resultClassList.add(PromotionResultModel.class);
		searchQuery.setResultClassList(resultClassList);
		final Integer totalSize = flexibleSearchService.search(searchQuery).getResult().size();
		return totalSize.intValue();
	}

	private int getActionTableSize()
	{
		final String query = "GET {" + AbstractPromotionActionModel._TYPECODE + "}";
		final FlexibleSearchQuery searchQuery = new FlexibleSearchQuery(query);
