...
public class DefaultCustomerFacadeTest
{
	...
	public void testForgottenPasswordForUid()
	{
		...
		verify(businessProcessService).createProcess(Mockito.matches("forgottenPassword-" + TEST_USER_UID + "-\\d{1,}"),
				eq(PROCESS_DEFINITION_NAME), Mockito.anyMapOf(String.class, Object.class));
	...
	public void testForgottenPasswordForCustomerId()
	{
		...
		verify(businessProcessService).createProcess(Mockito.matches("forgottenPassword-" + TEST_CUSTOMER_ID + "-\\d{1,}"),
				eq(PROCESS_DEFINITION_NAME), Mockito.anyMapOf(String.class, Object.class));
		verify(businessProcessService).startProcess(forgottenPasswordProcessModel);
	}
