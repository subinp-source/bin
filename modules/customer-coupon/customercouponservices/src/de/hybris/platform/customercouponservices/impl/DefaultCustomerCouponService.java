...
public class DefaultCustomerCouponService extends DefaultCouponService implements CustomerCouponService
{
	...
	@Override
	public Optional<CouponNotificationModel> saveCouponNotification(final String couponCode)
	{
		validateParameterNotNull(couponCode, COUPON_CODE_ERROR_MESSAGE);
		...
		final Optional<CouponNotificationModel> optional = couponNotificationList.stream()
				.filter(x -> x.getCustomer().getUid().equals(getUserService().getCurrentUser().getUid()))
				.findFirst();
	...
	@Override
	public List<PromotionSourceRuleModel> getPromotionSourcesRuleForProductCategories(final ProductModel product)
	{
