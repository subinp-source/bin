...
{
	private static final Logger LOG = Log.getLogger(IntegrationObjectItemsContext.class);
	...
	private void persist(final Set<IntegrationObjectItemModel> models)
	{
		models.forEach(model -> {
			...
			setIntegrationItemOnAttributes(model);
		});
		getIntegrationObject().setItems(models);
		integrationObjectsContext.givenExist(getIntegrationObject());
	}

	private void setIntegrationItemOnAttributes(final IntegrationObjectItemModel integrationItem)
	{
		integrationItem.getAttributes().forEach(attr -> {
			attr.setIntegrationObjectItem(integrationItem);
		});
	}
}
