...
public class DisableRuleValidationTest extends AbstractBundleValidationTest
{
    ...
    @Test
    public void shouldWarnAboutEmptyBundleTemplate() throws ImpExException {
        importString(
            "INSERT DisableProductBundleRule;id[unique=true];bundleTemplate(id, version, $catalogversion)[unique=true];conditionalProducts(code, $catalogversion);targetProducts(code, $catalogversion);$catalogversion;ruleType(code)[default=ANY]\n"
                + ";disable_NoBundleTemplate_ProductComponent1;;PRODUCT01,PRODUCT02;PRODUCT03\n"
        );
        Set<HybrisConstraintViolation> violations = validate("disable_NoBundleTemplate_ProductComponent1");
        assertThat(violations, hasItem(hasProperty("localizedMessage",
                                                   is("This disable rule has a target product PRODUCT03 which is not a part of the bundle template. Please either add the product PRODUCT03 to the bundle template list or remove this disable rule."))));
    }
